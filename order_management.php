    ...
    try {
        ...
        if (isset($_POST['delete_bulk'])) {
            if (isset($_POST['order_ids'])) {
                ...
                $deleteSql = "DELETE FROM `orders` WHERE `order_id` IN ($placeholders)";
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($_POST['order_id'])) {
                // `order_id` はデータベース内の列名であることを確認
                $search_conditions[] = "`order_id` = :order_id";
            }
        ...
        // 検索条件を考慮したSQLクエリの作成
        $sql = "SELECT `order_id`, `name`, `address`, `mail`, `title`, `amount`, `quantity`, `total_price`, `time`
        FROM `orders` $search_sql";
        ...
        echo "<input type='text' name='order_id' placeholder='注文番号'>";
